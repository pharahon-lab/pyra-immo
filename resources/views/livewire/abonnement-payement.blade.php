...
@script
    <script>
        ...
        $wire.on('test', (event) => {
            consol.log('test: ');
            consol.log('test: ' + event.detail.test);
            // $wire.$refresh()
...
<div class="px-12">
    <div class="flex flex-row w-full">
        ...
        <div class="flex flex-col justify-evenly p-4">
            <button id="1mois" wire:click="timeline({{ 1 }})" class="rounded-full border-2 semibold text-lg border-orange-600 px-6 mx-auto">1 Mois -> {{ $abonnement_type->price}}  F CFA</button>

            <button id="3mois" wire:click="timeline(3)" class="rounded-full border-2 semibold text-lg px-6 mx-auto">3 Mois -> {{ ($abonnement_type->price *3 ) *0.95 }}  F CFA</button>

            <button id="6mois"  onclick="selectPeriod(6)" class="rounded-full border-2 semibold text-lg px-6 mx-auto">6 Mois -> {{ ( $abonnement_type->price *6 ) * 0.90}}  F CFA</button>
            ...
            <button id="12mois" wire:click="timeline({{ 12 }})" class="rounded-full border-2 semibold text-lg px-6 mx-auto">1 an -> {{ ( $abonnement_type->price *12 ) *0.80 }}  F CFA</button>
